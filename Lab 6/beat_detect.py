...
0. Take mean of 200 mic samples to get ADC value of silence and set as MIC_OFFSET
1. Use interrupt to collect samples from mic at 8kHz rate.
2. Compute instantaneous energy E for 20ms window (N = 160).
3. Obtain sum of previous 50 E measurements as sum_energy, equivalent to 1s of signal.
...
mic = ADC(Pin('Y11'))
MIC_OFFSET = 0	# Initialise ADC reading of microphone for silence
dac = pyb.DAC(1, bits=12)	# Output voltage on X5 (BNC) for debugging
b_LED = LED(4)		# flash for beats on blue LED

N = 160				# size of sample buffer s_buf[]. 160 equivalent to 20ms window/epoch
s_buf = array('H', 0 for i in range(N))	# reserve buffer memory; initialised to 0
...
def offset():		# Compute and set MIC_OFFSET value
	oled.draw_text(0, 20, 'MIC_OFFSET:')
	oled.display()
	...
	mean = round(total/readings)
	
	return mean

MIC_OFFSET = offset()
oled.draw_text(0, 30, str(MIC_OFFSET))
oled.display()

def energy(buf):	# Compute energy of signal in buffer (in a 20ms epoch)
	sum = 0
...
M = 50						# number of instantaneous energy epochs to sum & average
BEAT_THRESHOLD = 2.0		# threshold for c to indicate a beat 
SILENCE_THRESHOLD = 1.3		# threshold for c to indicate silence
							# c = instantaneous energy / local average energy

# Initialise variables for main program loop
e_ptr = 0					# pointer to energy buffer storing M instant energy values
...
while True:				# Main program loop
	if buffer_full:		# semaphore signal from ISR - set if buffer is full
		...
		e_buf[e_ptr] = E		# overwrite earlest energy with most recent
		...
		if (pyb.millis()-tic > 500):	# if more than 500ms since last beat
			if (c > BEAT_THRESHOLD):	# look for a beat (threshold affects detection accuracy)
				flash()					# beat found, flash blue LED
				tic = pyb.millis()		# reset tic
		dac.write(0)					# sueful to see on scope, can remove
		buffer_full = False				# reset status flag; be ready for another 20ms period